function [opt_solution, opt_fun_val, iter_info, exit_flag] = BFGS_Method(obj_function, ...
...
for i = 1:iter_num
    ...
    f_lambada = @(temp_lambada)(obj_fun_1(current_sol+temp_lambada*direction));
    ...
    C_BFGS = ((p*p')/(p'*q))*(1+(q'*D_matrix*q)/(p'*q))-((D_matrix*q*p')+p*q'*D_matrix)/(p'*q);
    D_matrix = D_matrix + C_BFGS;
...
iteration_info = [iteration_info;[i+1,current_sol',[NaN,NaN]]];
